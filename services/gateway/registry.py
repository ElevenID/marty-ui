...
class ServiceRegistry:
    ...
    def _load_services(self) -> None:
        ...
        self._services = {
            ...
            "credential-templates": os.environ.get(
                "CREDENTIAL_TEMPLATE_SERVICE_URL", "http://localhost:8003"
            ),
            "trust-profiles": os.environ.get(
                "TRUST_PROFILE_SERVICE_URL", "http://localhost:8004"
            ),
            "issuance": os.environ.get("ISSUANCE_SERVICE_URL", "http://localhost:8005"),
            "applicant": os.environ.get(
                "APPLICANT_SERVICE_URL", "http://localhost:8006"
            ),
            "notifications": os.environ.get(
                "NOTIFICATION_SERVICE_URL", "http://localhost:8007"
            ),
            "compliance-profiles": os.environ.get(
                "COMPLIANCE_PROFILE_SERVICE_URL", "http://localhost:8008"
            ),
            "presentation-policies": os.environ.get(
                "PRESENTATION_POLICY_SERVICE_URL", "http://localhost:8009"
            ),
            "deployment-profiles": os.environ.get(
                "DEPLOYMENT_PROFILE_SERVICE_URL", "http://localhost:8010"
            ),
            "signing-keys": os.environ.get(
                "SIGNING_KEYS_SERVICE_URL", "http://localhost:8017"
            ),
            "flows": os.environ.get("FLOW_SERVICE_URL", "http://localhost:8011"),
            "verification": os.environ.get(
                "VERIFICATION_SERVICE_URL", "http://localhost:8012"
            ),
            "revocation-profiles": os.environ.get(
                "REVOCATION_PROFILE_SERVICE_URL", "http://localhost:8013"
            ),
            "device-registration": os.environ.get(
                "DEVICE_REGISTRATION_SERVICE_URL", "http://localhost:8014"
            ),
        }
...
ROUTE_CONFIG = {
    ...
    "/v1/organizations/invitations/validate": {
        "service": "auth",
        "requires_auth": False,
    },
    ...
    "/v1/vc-api": {"service": "gateway", "requires_auth": True},
    ...
    "/v1/flows/instances": {
        "service": "flows",
        "requires_auth": True,
    },  # wallet-facing /request + /submit handled by _WALLET_PUBLIC regex
    "/v1/flows/siop/submit": {
        "service": "flows",
        "requires_auth": False,
    },  # SIOPv2 wallet-facing
    "/v1/flows/siop": {
        "service": "flows",
        "requires_auth": True,
