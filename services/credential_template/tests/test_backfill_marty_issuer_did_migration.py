...
MIGRATION_PATH = (
    ...
    / "migrations"
    / "versions"
    / "20260729_0001_backfill_marty_issuer_did.py"
)


def _load_migration():
    spec = importlib.util.spec_from_file_location(
        "backfill_marty_issuer_did",
        MIGRATION_PATH,
    )
...
def test_upgrade_binds_only_active_remote_marty_templates_missing_a_did(
    monkeypatch,
) -> None:
    ...
    assert "key_access_mode = 'REMOTE_SIGNING'" in sql
    assert "nullif(trim(issuer_profile_id), '') IS NOT NULL" in sql
    assert parameters == {
