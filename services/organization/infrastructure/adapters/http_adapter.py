...
from ...domain.entities import JoinMechanism, MemberStatus, OrganizationType
...
def _organization_creation_enabled() -> bool:
    ...
    return (
        os.environ.get("ORGANIZATION_CREATION_ENABLED", "true").strip().lower()
...
class CreateOrganizationRequest(BaseModel):
    ...
    def validate_admission(self) -> "CreateOrganizationRequest":
...
class UpdateOrganizationRequest(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_update(self) -> "UpdateOrganizationRequest":
...
async def create_organization(
    ...
    if not _organization_creation_enabled():
        raise HTTPException(
            status_code=403,
            detail="Organization creation is disabled for this deployment",
        )
    ...
    try:
        org = await use_case.create_organization(
            CreateOrganizationCommand(
                ...
                join_mechanism=JoinMechanism(request.join_mechanism),
        ...
        if member_repo is not None and hasattr(member_repo, "get_by_user_and_org"):
            owner_membership = await member_repo.get_by_user_and_org(
                user_id, str(org.id)
            )
...
@router.get(
    "", response_model=list[OrganizationResponse], response_model_exclude_none=True
)
...
async def discover_organizations(
    search: str | None = Query(
        default=None, description="Search by name or display name"
    ),
    org_type: str | None = Query(
        default=None, description="Filter by organization type"
    ),
    join_mechanism: str | None = Query(
        default=None,
        description="Filter by join mechanism (open, code, invite, domain)",
    ),
    limit: int = Query(default=100, le=1000),
    offset: int = Query(default=0, ge=0),
    use_case: OrganizationUseCase = Depends(get_org_use_case),
...
async def get_my_organizations(
    ...
    return [
        _org_to_response(org, membership=membership)
        for org, membership in org_memberships
    ]


@router.get(
    "/{org_id}", response_model=OrganizationResponse, response_model_exclude_none=True
)
    ...
    try:
        org = await use_case.update_organization(
            UpdateOrganizationCommand(
                ...
                display_name=request.display_name,
                org_type=(
                    _normalize_org_type(request.org_type)
                    if request.org_type is not None
                    else None
                ),
                description=request.description,
                contact_email=request.contact_email,
                ...
                visibility=request.visibility,
                join_mechanism=(
                    JoinMechanism(request.join_mechanism)
                    if request.join_mechanism is not None
                    else None
                ),
                requires_approval=request.requires_approval,
...
@router.get(
    "/{org_id}/team/snapshot",
    response_model=TeamSnapshotResponse,
    response_model_exclude_none=True,
)
async def get_team_snapshot(
    ...
    active_members = [
        member for member in members if member.status == MemberStatus.ACTIVE
    ]
    pending_members = [
        member
    ...
    return TeamSnapshotResponse(
        ...
        pending_invites=[
            _member_dashboard_summary(member) for member in pending_members
        ],
...
async def get_organization_environment(
    ...
    return OrganizationEnvironmentResponse(
        organization_id=org_id,
        environment=_normalize_dashboard_environment(
            (org.settings or {}).get("environment")
        ),
    )
...
@router.patch(
    "/{org_id}/environment",
    response_model=OrganizationEnvironmentResponse,
    response_model_exclude_none=True,
)
async def update_organization_environment(
    ...
    return OrganizationEnvironmentResponse(
        organization_id=org_id, environment=environment
    )
...
@router.post(
    "/{org_id}/members", response_model=MemberResponse, response_model_exclude_none=True
)
...
async def update_member(
    ...
    try:
        ...
        if member.user_id and hasattr(http_request.app.state, "org_client"):
            ...
            logger.info(
                f"Invalidated membership cache for user {member.user_id} in org {org_id}"
            )
...
def _org_to_response(org, membership=None) -> OrganizationResponse:
    return OrganizationResponse(
        ...
        visibility="PUBLIC" if org.is_discoverable else "PRIVATE",
...
def _org_to_lifecycle_response(org) -> OrganizationLifecycleResponse:
    ...
    data_retention_mode = (
        ...
        else str(settings.get("data_retention_mode") or "standard")
    )
    audit_retention_days = _coerce_positive_int(
        settings.get("audit_retention_days")
        if hosted_pilot_enabled or data_retention_mode != "standard"
        else None,
...
def _api_key_to_response(api_key) -> ApiKeyResponse:
    return ApiKeyResponse(
        ...
        deployment_profile_id=str(api_key.deployment_profile_id)
        if api_key.deployment_profile_id
        else None,
