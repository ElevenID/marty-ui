...
class TrustApiAdapter {
  ...
  async getTrustConfig(orgId) {
    try {
      const data = await this._request(`/v1/organizations/${orgId}/trust-config`);
  ...
  async generateKey(orgId, options = {}) {
    ...
    return this._request(`/v1/organizations/${orgId}/trust-config/generate-key`, {
      method: 'POST',
      body: JSON.stringify(payload),
    });
  }
  ...
  async testKeyConnection(orgId, keyConfig) {
    try {
      ...
      if (keyConfig.source === IssuerKeySource.KMS && keyConfig.kmsArn) {
        ...
        return this._request(`/v1/organizations/${orgId}/trust-config/test-kms`, {
          method: 'POST',
          body: JSON.stringify({
