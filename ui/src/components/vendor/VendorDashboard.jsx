...
function ComplianceBadge({ profile }) {
  ...
  const getNextSteps = () => {
    if (profile.compliance_status === 'COMPLIANT') {
      return [t('vendorDashboard.compliance.nextSteps.allOperational'), t('vendorDashboard.compliance.nextSteps.noActionRequired')];
    }
    if (profile.compliance_status === 'NEEDS_ATTENTION') {
      return [
        profile.certificate_expiry_days && profile.certificate_expiry_days < 30
          ? t('vendorDashboard.compliance.nextSteps.certificateExpiring', { days: profile.certificate_expiry_days })
          : null,
        profile.trust_list_age_hours && profile.trust_list_age_hours > 48
          ? t('vendorDashboard.compliance.nextSteps.trustListStale')
          : null,
      ].filter(Boolean);
    }
    if (profile.compliance_status === 'SETUP_REQUIRED') {
      return [
        t('vendorDashboard.compliance.nextSteps.completeTrustProfile'),
        t('vendorDashboard.compliance.nextSteps.uploadCertificates'),
        t('vendorDashboard.compliance.nextSteps.testVerification'),
      ];
    }
    return [];
  };
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flexGrow: 1 }}>
            ...
            <Box sx={{ flexGrow: 1 }}>
              ...
              <Typography variant="body2" color="text.secondary">
                {t(`vendorDashboard.compliance.statuses.${profile.compliance_status.toLowerCase()}`)}
              </Typography>
            </Box>
          </Box>
        ...
        <Collapse in={expanded} timeout="auto" unmountOnExit>
          ...
          {nextSteps.length > 0 && (
            <Box sx={{ mb: 2 }}>
              <Typography variant="subtitle2" fontWeight="bold" gutterBottom>
                {t('vendorDashboard.compliance.nextStepsTitle')}
              </Typography>
          ...
          >
            <Typography variant="caption" fontWeight="bold" display="block" gutterBottom>
              {t('vendorDashboard.compliance.advancedDetailsTitle')}
            </Typography>
            <Grid container spacing={1}>
              <Grid item xs={6}>
                <Typography variant="caption" color="text.secondary">
                  {t('vendorDashboard.compliance.advancedDetails.framework')}
                </Typography>
                <Typography variant="body2">{profile.profile_type}</Typography>
              </Grid>
              <Grid item xs={6}>
                <Typography variant="caption" color="text.secondary">
                  {t('vendorDashboard.compliance.advancedDetails.technicalName')}
                </Typography>
              ...
              {profile.use_case_tags && profile.use_case_tags.length > 0 && (
                <Grid item xs={12}>
                  <Typography variant="caption" color="text.secondary" display="block">
                    {t('vendorDashboard.compliance.advancedDetails.useCases')}
                  </Typography>
              ...
              {profile.auto_generated && (
                <Grid item xs={12}>
                  <Chip
                    label={t('vendorDashboard.compliance.advancedDetails.autoGenerated')}
                    size="small"
                    icon={<InfoIcon />}
                    variant="outlined"
                  />
          ...
          <Box sx={{ mt: 2, display: 'flex', gap: 1 }}>
            ...
            >
              {t('vendorDashboard.compliance.buttons.configure')}
            </Button>
            {profile.compliance_status !== 'COMPLIANT' && (
              ...
              >
                {t('vendorDashboard.compliance.buttons.fixIssues')}
              </Button>
            )}
          </Box>
        </Collapse>
    ...
    const fetchTrustProfiles = async () => {
      try {
        ...
        const response = await fetch(
          `${API_BASE_URL}/v1/trust-profiles?organization_id=${organizationId}`,
          { credentials: 'include' }
        );
    ...
    <Box sx={{ p: 3 }} data-testid="vendor-dashboard">
      ...
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant="body1" color="textSecondary" data-testid="welcome-message">
          {t('vendorDashboard.welcomeMessage', { name: user?.given_name || user?.email })}
        </Typography>
        {organizationId && (
          <Typography variant="caption" color="textSecondary" data-testid="org-id">
            {t('vendorDashboard.organizationId', { id: organizationId })}
          </Typography>
        )}
      </Box>
      ...
      <Grid container spacing={3} sx={{ mb: 4 }} data-testid="stats-grid">
        <Grid item xs={12} sm={6} md={3}>
          <StatCard
            title={t('vendorDashboard.stats.apiKeys')}
            value={stats.apiKeys}
            icon={<VpnKeyIcon />}
            color="primary"
        ...
        <Grid item xs={12} sm={6} md={3}>
          <StatCard
            title={t('vendorDashboard.stats.activeApplicants')}
            value={stats.activeApplicants}
            icon={<PeopleIcon />}
            color="secondary"
            trend={t('vendorDashboard.stats.trend', { count: 3, period: 'week' })}
            testId="stat-active-applicants"
          />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <StatCard
            title={t('vendorDashboard.stats.credentialsIssued')}
            value={stats.credentialsIssued}
            icon={<CredentialIcon />}
            color="success"
            trend={t('vendorDashboard.stats.trend', { count: 12, period: 'month' })}
            testId="stat-credentials-issued"
          />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <StatCard
            title={t('vendorDashboard.stats.processingFee')}
      ...
      {!loadingProfiles && trustSetupComplete === false && (
        <Alert
          ...
          action={
            ...
            >
              {t('vendorDashboard.alerts.setupIncomplete.action')}
            </Button>
          }
          sx={{ mb: 3 }}
        ...
        >
          <Typography variant="body2">
            <strong>{t('vendorDashboard.alerts.setupIncomplete.title')}</strong> {t('vendorDashboard.alerts.setupIncomplete.message')}
          </Typography>
        </Alert>
      )}
        ...
        <Paper sx={{ p: 3, mb: 3 }} data-testid="compliance-panel">
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              <Typography variant="h6">{t('vendorDashboard.compliance.sectionTitle')}</Typography>
            </Box>
            <Button
              size="small"
              variant="text"
            ...
            >
              {t('vendorDashboard.compliance.manageButton')}
            </Button>
          </Box>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
            {t('vendorDashboard.compliance.description')}
          </Typography>
          ...
          <Box sx={{ mt: 2, p: 2, bgcolor: 'grey.50', borderRadius: 1 }}>
            <Typography variant="caption" fontWeight="bold" display="block" gutterBottom>
              {t('vendorDashboard.compliance.overallStatusTitle')}
            </Typography>
            <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
              <Chip
                label={t('vendorDashboard.compliance.overallStatus.compliant', { count: trustProfiles.filter(p => p.compliance_status === 'COMPLIANT').length })}
                color="success"
                size="small"
                icon={<CheckCircleIcon />}
              />
              <Chip
                label={t('vendorDashboard.compliance.overallStatus.needsAttention', { count: trustProfiles.filter(p => p.compliance_status === 'NEEDS_ATTENTION').length })}
                color="warning"
                size="small"
                icon={<WarningIcon />}
              />
              <Chip
                label={t('vendorDashboard.compliance.overallStatus.setupRequired', { count: trustProfiles.filter(p => p.compliance_status === 'SETUP_REQUIRED').length })}
                color="error"
                size="small"
                icon={<ErrorIcon />}
              />
      ...
      {loadingProfiles && (
        <Paper sx={{ p: 3, mb: 3 }}>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mt: 2 }}>
            {t('vendorDashboard.compliance.loadingStatus')}
          </Typography>
        </Paper>
      )}
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} md={6}>
          <Paper sx={{ p: 2 }} data-testid="quick-actions-panel">
            <Typography variant="h6" gutterBottom>
              {t('vendorDashboard.quickActions.title')}
            </Typography>
            <Divider sx={{ mb: 2 }} />
            <List disablePadding>
              <QuickAction
                title={t('vendorDashboard.quickActions.apiKeys.title')}
                description={t('vendorDashboard.quickActions.apiKeys.description')}
                icon={<VpnKeyIcon color="primary" />}
                to="/vendor/api-keys"
                testId="action-api-keys"
              />
              <QuickAction
                title={t('vendorDashboard.quickActions.inviteApplicants.title')}
                description={t('vendorDashboard.quickActions.inviteApplicants.description')}
                icon={<PeopleIcon color="secondary" />}
                to="/vendor/invite"
                testId="action-invite-applicants"
              />
              <QuickAction
                title={t('vendorDashboard.quickActions.devices.title')}
                description={t('vendorDashboard.quickActions.devices.description')}
                icon={<DevicesIcon color="info" />}
                to="/vendor/devices"
                testId="action-devices"
              />
              <QuickAction
                title={t('vendorDashboard.quickActions.fees.title')}
                description={t('vendorDashboard.quickActions.fees.description')}
                icon={<AttachMoneyIcon color="warning" />}
                to="/vendor/fees"
                testId="action-fees"
              />
              <QuickAction
                title={t('vendorDashboard.quickActions.apiDocs.title')}
                description={t('vendorDashboard.quickActions.apiDocs.description')}
        ...
        <Grid item xs={12} md={6}>
          <Paper sx={{ p: 2 }} data-testid="org-settings-panel">
            <Typography variant="h6" gutterBottom>
              {t('vendorDashboard.organizationSettings.title')}
            </Typography>
            <Divider sx={{ mb: 2 }} />
            <List disablePadding>
              <QuickAction
                title={t('vendorDashboard.organizationSettings.webhooks.title')}
                description={t('vendorDashboard.organizationSettings.webhooks.description')}
                icon={<WebhookIcon color="primary" />}
                to="/vendor/webhooks"
                testId="action-webhooks"
              />
              <QuickAction
                title={t('vendorDashboard.organizationSettings.credentials.title')}
                description={t('vendorDashboard.organizationSettings.credentials.description')}
                icon={<CredentialIcon color="success" />}
                to="/vendor/credentials"
                testId="action-credentials"
              />
              <QuickAction
                title={t('vendorDashboard.organizationSettings.settings.title')}
                description={t('vendorDashboard.organizationSettings.settings.description')}
      ...
      <Paper sx={{ p: 2, mt: 3 }} data-testid="subscription-panel">
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Box>
            <Typography variant="h6">{t('vendorDashboard.subscription.title')}</Typography>
            <Typography variant="body2" color="textSecondary">
              {t('vendorDashboard.subscription.planInfo', { plan: 'Professional', date: 'Jan 15, 2026' })}
            </Typography>
          </Box>
          <Button variant="outlined" component={Link} to="/vendor/subscription" data-testid="manage-subscription-btn">
            {t('vendorDashboard.subscription.manageButton')}
          </Button>
        </Box>
      </Paper>
    </Box>
