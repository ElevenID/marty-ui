...
const FlowManager = () => {
  ...
  const [executeFlowDialog, setExecuteFlowDialog] = useState(false);
  ...
  const handleExecuteFlow = async (flowId, context = {}) => {
    try {
      await flowsApi.startFlowExecution(flowId, context);
      setExecuteFlowDialog(false);
      setSnackbar({
        open: true,
      ...
      setSnackbar({
        open: true,
        message: `Failed to start flow: ${err.message}`,
        severity: 'error',
      });
    }
  };
  ...
  const handleReject = async (execution, reason) => {
    try {
      await flowsApi.rejectFlowExecution(
        execution.flow_id,
        execution.id,
        { reason, notes: 'Rejected via UI' }
      );
      setApprovalDialog(false);
      setSnackbar({
        open: true,
    ...
    <Box>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        ...
        <Button
          ...
          color="primary"
          startIcon={<AddIcon />}
          onClick={() => setCreateFlowDialog(true)}
        >
          Create Flow
        </Button>
      </Box>
