...
cmd_setup_secrets() {
  ...
  local issuance_api_key canvas_credentials_shared_secret openbao_service_token license_key
  ...
  canvas_credentials_api_token="$(resolve_secret_input CANVAS_CREDENTIALS_API_TOKEN)"
  ...
  kubectl create secret generic marty-secrets \
    ...
    --from-literal=CANVAS_CREDENTIALS_API_TOKEN="$canvas_credentials_api_token" \
