...
"${executables[0]}" --list | grep -Fx 'resolved_kubernetes_runtime::prepared_cleanup_retains_modified_bytes_until_exact_owned_content_is_restored: test'
"${executables[0]}" --list | grep -Fx 'base_profile_envoy_composition_isolated: test'
"${executables[0]}" --list | grep -Fx 'base_profile_envoy_composition_child: test'
"${executables[0]}" --list | grep -Fx 'envoy_actual_image_validates_candidate: test'
...
"${executables[0]}" --list | grep -Fx 'canvas_base_review_gateway_matches_corrected_published_schema: test'
...
"${executables[0]}" --list | grep -Fx 'renewal_fresh_packaged_main_delivers_both_encryption_modes: test'
"${executables[0]}" --list | grep -Fx 'renewal_packaged_main_recovers_historical_keyed_offer: test'
"${executables[0]}" --list | grep -Fx 'didcomm_renewal_gateway_selects_native_with_required_owner_read: test'
"${executables[0]}" --list | grep -Fx 'didcomm_renewal_canvas_preserves_real_association_and_delivery_phases: test'
...
"${executables[0]}" --list | grep -Fx 'flow_actual_main_boots_rendered_base_and_preserves_public_admission: test'
"${executables[0]}" --list | grep -Fx 'didcomm_admission_recovery::flow_consumer::public_startup::owned_process_output_and_early_exit_cleanup_are_verified: test'
"${executables[0]}" --list | grep -Fx 'didcomm_admission_recovery::flow_consumer::public_startup::loader_capture_preserves_values_and_removes_file_alias_before_direct_spawn: test'
