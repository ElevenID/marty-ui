...
**Status:** Waves one through three, the 31-route Rust Canvas cutover, and the canonical Rust verification-image implementation are merged, but the verifier cutover is on repair hold. `marty-credentials@v0.1.72` is immutable and ineligible, `marty-integration-tests@v1.2.76` is stale and ineligible, `v1.2.77` is containment-harness evidence only, `marty-ui@v1.1.209` is a quarantined partial publication, and unsigned tag `marty-ui@v1.1.210` is a quarantined release attempt whose UI image remains publicly accessible. The affected workflows are disabled, no `v1.1.210` services/migrations image or GitHub release exists, no replacement UI version is selected, and production is unchanged
...
10. The Canvas, mdoc, Canvas Credentials, base verifier implementation, verifier
   ...
   verifier release-binder stream and Credentials deletion evidence until the
   corrected release/pin gates complete. Finish
