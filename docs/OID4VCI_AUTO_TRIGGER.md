...
MIP 0.4.0 can generate a pre-authorized OID4VCI offer after an organization reviewer approves an application. The application service owns claim derivation and invokes only an active custom Flow extension that explicitly handles `APPLICATION_APPROVED` for the application's Credential Template.
