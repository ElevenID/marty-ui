...
| 10. OAuth revocation failure and owner fences | [Seven published-process DELETE observations](canvas-worker-oauth-revocation.md) now have exact-head native Linux qualification at31355; two separately frozen replacement-owner races and one marker-write failure have native replay implemented, qualification pending | Qualify replacement-owner success/retry preservation and marker-failure cleanup, then remaining Retry-After and fence dimensions. Explicitly retain Rust's stronger atomic cleanup. Windows transport diagnostics are not native HTTPS parity. |
