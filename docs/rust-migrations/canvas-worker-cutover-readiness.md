...
| --- | --- | --- |
| 1. Environment parsing, bounds, malformed startup | 133 startup vectors, PostgreSQL consumer cycles and 24 packaged startup cases; [deployed LOG_LEVEL repair](canvas-worker-logging-configuration.md) has 16 frozen threshold cases and seven actual invalid-process checks | Qualify the logging repair and nine-case image preflight on the exact head; execute remaining deployed entrypoint/configuration shapes, not only configuration factories. |
