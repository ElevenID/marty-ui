...
| 10. OAuth revocation failure and owner fences | [Seven published-process DELETE observations](canvas-worker-oauth-revocation.md) have exact-head native Linux qualification at31355; two replacement-owner races, one marker-write failure and eight Retry-After cases are separately frozen with native replay implemented, qualification pending | Qualify these native extensions, then audit selection, retry/counter and ownership requirements against the normative contract before closure. Explicitly retain Rust's stronger atomic cleanup. Windows transport diagnostics are not native HTTPS parity. |
