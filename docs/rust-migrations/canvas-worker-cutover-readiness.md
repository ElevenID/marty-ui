...
| 10. OAuth revocation failure and owner fences | [Seven published-process DELETE observations plus two separately frozen replacement-owner races](canvas-worker-oauth-revocation.md) have independent captures and unchanged regeneration; native process/SQL/HTTPS replay is implemented but awaits Linux qualification | Qualify native transport and replacement-owner success/retry preservation, then remaining Retry-After, patch failure and fence dimensions. Explicitly retain Rust's stronger atomic cleanup. Windows transport diagnostics are not native HTTPS parity. |
