...
ignores stale progress callbacks and callbacks to terminal jobs, returning the
unchanged job idempotently as recorded in
`contracts/passport-webhook-progress-behavior.json`. This protects both legacy
and KMS-verified callbacks from replay-based status reversal. Invalid signatures
