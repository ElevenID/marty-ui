...
`marty-integration-tests@v1.2.77` are ineligible. `marty-ui@v1.1.209` and
unsigned tag `marty-ui@v1.1.210` are quarantined partial publications. All
affected release workflows remain disabled, the UI release environment
prevents self-approval, and no replacement version is selected. The separate
public Python binding and still-used Credentials adapter remain supported, and
...
and prevents self-approval by the dispatching identity.
...
`bd3abf0792bad5c61faa2ff3b0f56fb4df0807d7`. No immutable post-fix integration
release exists. A post-merge audit also reproduced a frozen `v0.1.71`
...
The forward harness correction must remove the generic retry, resample an
incompatible ID only before submission and only for exact allowlisted frozen
artifacts, retain only a sanitized resample count, and fail closed on bounded
exhaustion. Future or ready Rust artifacts receive no allowance and must return
an immediate canonical terminal result for every submitted session. The
product-side repair prefixes the adapter transaction identifier before it
enters Core.
...
replacement release version is selected. Repair proceeds in order: publish a
corrected Credentials release; publish an exact-reviewed forward harness
release from the corrected integration `main`; select and prepare a then-unused
Rust artifact; create a new exact integration pin; pass the full differential
matrix; merge that exact pin tree through protection; publish and independently
verify a distinct immutable post-pin integration release; and only then bind
that release with corrected Credentials into a later beta-only aggregate. The
preliminary forward harness release is not an aggregate pin. `v0.1.72`,
`v1.2.76`, `v1.2.77`, `v1.1.209`, and `v1.1.210` remain nondeployable
