...
The `X-User-Id` gateway context remains the HTTP identity input. Missing identity returns `401`. Missing membership or permission returns `403`. An unavailable authorization, storage, or canonical status backend returns `503`; it never falls back to an in-process allow decision.
...
- Add authenticated internal allocation and lifecycle routes.
- Add public Bitstring and Token Status List document routes with canonical Rust bytes.
