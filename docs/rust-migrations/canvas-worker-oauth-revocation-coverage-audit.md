...
| Owner-fenced success and retry writes | Two held-response replacement-owner process captures; full replacement-row and ciphertext equality | Native replay implemented; hosted qualification pending |
| Failed disconnected projection still cleans up tokens | Actual platform UPDATE barrier and synthetic write-failure capture; separate qualified privacy/counter replay | New whole-process native marker-failure replay awaits hosted qualification |
...
| Due selection, status/lease eligibility and order | Real repository contract checks one deferred row then its acquisition; published process cases each start from one due connection | Nonempty multi-row selection/order and excluded status/lease cases are not established by empty-queue range tests |
| Batch cap and lease duration | Frozen environment/range corpora and actual shared query implementation; range observer verifies bounded limit | Exact nonempty cap/selection and acquired-duration boundaries need focused evidence, not an empty-queue claim |
