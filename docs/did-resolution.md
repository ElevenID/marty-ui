...
the configured internal resolver first. Public network fallback is disabled by
default and must be enabled by deployment configuration, never by a credential
or API request.
...
service-network destinations. Application validation is defense in depth, not a
replacement for a firewall or controlled outbound proxy.
