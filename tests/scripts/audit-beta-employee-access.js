...
async function listResources(page, pathName) {
  const separator = pathName.includes('?') ? '&' : '?';
  const body = await requireJson(
    page,
    `${pathName}${separator}organization_id=${encodeURIComponent(ORG_ID)}`,
  );
...
async function ensureActiveResource(page, {
  ...
  const matches = (await listResources(page, collectionPath))
...
async function ensureApplicantProfile(page, email) {
...
async function findCurrentCredential(page, credentialTemplateId, startedAt) {
  return waitFor(() => page.evaluate(async ({ organizationId, templateId, lowerBound }) => {
    ...
    return records
      ...
      || null;
  }, { organizationId: ORG_ID, templateId: credentialTemplateId, lowerBound: startedAt }));
}
...
async function cleanup(page, applicationId, credentialId) {
  ...
  if (credentialId) {
    const response = await browserJson(page, `/v1/issued-credentials/${encodeURIComponent(credentialId)}/revoke`, {
      method: 'POST',
      body: JSON.stringify({ reason: 'D-04 release qualification cleanup' }),
    });
  ...
  if (applicationId) {
    const response = await browserJson(
      page,
      `/v1/organizations/${encodeURIComponent(ORG_ID)}/applicants/${encodeURIComponent(applicationId)}/withdraw`,
      {
        method: 'POST',
        body: JSON.stringify({ reason: 'D-04 release qualification cleanup' }),
      },
...
async function main() {
  ...
  try {
    ...
    await ensureApplicantProfile(applicantPage, applicantEmail);
    ...
    const credential = await findCurrentCredential(page, configuration.credential.id, startedAt);
