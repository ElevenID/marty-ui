...
def test_manifest_snapshots_all_coordinated_repositories(tmp_path: Path) -> None:
    ...
    manifest = create_manifest(workspace, "mip-0.4.0-local-test", output, snapshots)
    ...
    assert json.loads(output.read_text(encoding="utf-8"))["mip_version"] == "0.4.0"
...
def test_manifest_verification_rejects_worktree_drift(tmp_path: Path) -> None:
    ...
    for name in REPOSITORIES:
        ...
        if name == "marty-blog":
            ...
            (repo / "src" / "index.js").write_text("export {};", encoding="utf-8")
    output = tmp_path / "manifest.json"
    create_manifest(workspace, "mip-0.4.0-local-test", output, tmp_path / "snapshots")
