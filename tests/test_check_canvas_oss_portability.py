...
def test_runtime_binding_requires_deployment_and_source_snapshot(tmp_path: Path) -> None:
    ...
    context = write(
        ...
        runtime_context("mip-0.4.0-local-test", source_id),
    )
    write(
        tmp_path / "source-manifest.json",
        {
            ...
            "release_version": "mip-0.4.0-local-test",
    ...
    deployment = write(
        tmp_path / "local-deployment-manifest.json",
        {
            ...
            "services_marker": {
                ...
                "release_version": "mip-0.4.0-local-test",
                "marty_ui_sha": source_id,
                "deployment_release_marker": "mip-0.4.0-local-test",
                "image_digests": {
                    "gateway": "sha256:" + "1" * 64,
            ...
            "images": [
                {
                    "container": container,
                    "configured_image": f"elevenid-local/{image_service}:mip-0.4.0-local-test",
                    "image_id": image_id,
                    "status": "running",
                }
...
def test_runtime_binding_rejects_current_ui_only_reused_backend_manifest(tmp_path: Path) -> None:
    ...
    revision = "3" * 40
    release = "mip-0.4.0-local-current"
    context = write(
        tmp_path / "runtime.json",
...
def test_full_finalize_requires_all_oss_cases_and_hosted_classification(tmp_path: Path) -> None:
    ...
    args = Namespace(
        ...
        runtime_context=write(
            tmp_path / "runtime.json",
            runtime_context("mip-0.4.0-local-test", "b" * 40),
        ),
