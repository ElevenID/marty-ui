...
`license_public_key` is not sensitive in the same way as the license key itself, but the current reference stack keeps it file-backed so the runtime contract stays consistent across Docker Compose and Kubernetes-style `*_FILE` delivery.
