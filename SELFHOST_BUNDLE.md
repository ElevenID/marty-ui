...
Set `SELFHOST_IMAGE_TAG` to the released immutable version you want customers to run. Do not use `latest` or `--build` with the customer bundle.
...
Bundle generation fails if the staged customer output contains Docker `build:` keys or mutable image tag aliases such as `latest`, `prod`, `main`, or `dev`.
...
4. Replace the required secret placeholder files, including `license_key`.
...
- The self-host services and migrations validate the signed license before startup using the Marty issuer public key embedded in the runtime image.
