...
use marty_release_evidence::{
    demo_qualification::{
        validate_demo_qualification, ExpectedQualification, MAX_REPORT_BYTES, MAX_REVIEW_BYTES,
    },
...
fn run() -> Result<String, &'static str> {
    ...
    if args.len() != 13 {
        return Err("usage: validate-demo-qualification RUN_FILE REPORT_FILE COMMENT_FILE PERMISSION_FILE RUN_ID RECORDER_SHA REVIEW_RECORD_ID VERSION BETA_ORIGIN UI_SHA SOURCE_ID DEPLOYMENT_SHA256 STACK_SHA256");
    }
    let expected = ExpectedQualification {
        ...
        recorder_sha: &args[5],
        review_record_id: args[6]
            .parse()
            .map_err(|_| "invalid maintainer review record ID")?,
        release_version: &args[7],
        beta_origin: &args[8],
        ui_sha: &args[9],
        source_id: &args[10],
        deployment_sha256: &args[11],
        stack_sha256: &args[12],
    };
    let run = read_file(&args[0], MAX_RUN_BYTES)?;
    let report = read_file(&args[1], MAX_REPORT_BYTES)?;
    let comment = read_file(&args[2], MAX_REVIEW_BYTES)?;
    let permission = read_file(&args[3], MAX_REVIEW_BYTES)?;
    let verified = validate_demo_qualification(&run, &report, &comment, &permission, &expected)?;
